...
const emit = defineEmits(["close", "template-updated"]);
...
// Watch for template prop changes and populate form
...
const populateForm = (template) => {
  ...
  gradeLevel.value = template.grade_level || "";
  description.value = template.description || "";
  totalItems.value = template.total_items || 50;
  
  cognitivePercentages.value = {
    remembering: template.percentage_remembering || 0,
    understanding: template.percentage_understanding || 0,
    applying: template.percentage_applying || 0,
    analyzing: template.percentage_analyzing || 0,
    evaluating: template.percentage_evaluating || 0,
    creating: template.percentage_creating || 0,
  };
  
  // Map topics from tos_template_topics and add isExpanded property
  const templateTopics = template.tos_template_topics || template.topics || [];
  topics.value = templateTopics.map(topic => ({
    ...topic,
    isExpanded: false
  }));
};
...
const nextStep = () => {
  ...
  if (currentStep.value === 2 && !isStep2Valid.value) {
    alert("Cognitive level percentages must total 100%");
    return;
  }
...
const autoDistributeTopics = () => {
  ...
  topics.value.forEach((topic, index) => {
    ...
    // Calculate cognitive levels without exceeding total
    const levels = ['remembering', 'understanding', 'applying', 'analyzing', 'evaluating', 'creating'];
    let allocated = 0;
...
const autoDistributeTopicCognitive = (index) => {
  ...
  const totalTopicItems = topic.total_items;
  ...
  levels.forEach((level, idx) => {
    if (idx === levels.length - 1) {
      ...
      topic[`items_${level}`] = totalTopicItems - allocated;
    } else {
      const items = Math.floor(totalTopicItems * (cognitivePercentages.value[level] / 100));
      topic[`items_${level}`] = items;
      allocated += items;
    }
  });
...
const saveTemplate = async () => {
  ...
  try {
    ...
    const result = await api.tosApi.updateTOSTemplate(props.template.id, templateData);
    if (result.error) {
    ...
    console.error("Error updating TOS template:", error);
    errors.value.save = error.message || "Failed to update TOS template";
  } finally {
          ...
          >
            ...
            <div class="bg-gradient-to-r from-green-600 to-green-700 px-6 py-4">
              <div class="flex items-center justify-between">
                ...
                <button
                  @click="close"
                  class="text-white hover:text-green-100 transition-colors duration-200"
                >
              ...
              <!-- Progress Bar -->
              <div class="mt-4 bg-green-800 rounded-full h-2">
                <div
            ...
            <!-- Body - Reuse the same steps as CreateTOSModal -->
            <div class="px-6 py-6 max-h-[60vh] overflow-y-auto">
                ...
                <div class="space-y-4">
                  ...
                  <div>
                    ...
                    <textarea
                      v-model="description"
                      rows="3"
                      class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-transparent resize-none"
                    ></textarea>
                  </div>
                </div>
              </div>
              ...
              <div v-show="currentStep === 3">
                ...
                <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
                  Add topics covered in the test. Total items must equal {{ totalItems }}.
                </p>
            ...
            <div class="bg-gray-50 dark:bg-gray-900/50 px-6 py-4 flex items-center justify-between border-t border-gray-200 dark:border-gray-700">
              ...
              <div class="flex items-center space-x-3">
                ...
                <button
                  ...
                  class="px-6 py-2 bg-green-600 hover:bg-green-700 disabled:bg-gray-400 disabled:cursor-not-allowed text-white rounded-lg transition-colors duration-200"
                >
